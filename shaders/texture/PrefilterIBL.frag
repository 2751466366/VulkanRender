...
layout(set = 0, binding = 2) uniform transformData {
    float roughness;
    float cubeResolutionWidth;
    float cubeResolutionHeight;
} uniData;
...
void main()
{
    ...
    for(uint i = 0u; i < numSamples; ++i)
    {
        ...
        vec3 H = computeImportanceSampleGGX(Xi, uniData.roughness, N);
        ...
        if(NdotL > 0.0f)
        {
            float D = computeDistributionGGX(N, H, uniData.roughness);
            ...
            float saTexel  = 4.0f * PI / (6.0f * uniData.cubeResolutionWidth * uniData.cubeResolutionHeight);
            float saSample = 1.0f / (float(numSamples) * probaDistribFunction + 0.0001f);
            float mipLevel = uniData.roughness == 0.0f ? 0.0f : 0.5f * log2(saSample / saTexel);
